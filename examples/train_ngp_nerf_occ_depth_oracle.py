"""
NGP Occ NeRF trained with a frozen 2D depth/opacity sampling oracle (oracle_only).
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Override default margin (0.1 synthetic / 0.5 for 360)",
    )
    parser.add_argument(
        "--opacity_threshold",
        type=float,
    ...
    def oracle_bounds_from_maps(depth, opacity):
        ...
        t_min = torch.clamp(depth - depth_margin, min=near_plane)
        t_max = depth + depth_margin
    ...
    log_print(
        ...
        f"depth_margin={depth_margin} opacity_threshold={args.opacity_threshold} "
